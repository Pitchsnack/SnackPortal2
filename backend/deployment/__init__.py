...
Every service package (``api_gateway``, ``auth_router``, ``database_router``, ``control_plane``,
...
Eight of the nine HTTP edges are composable entirely inside their owning service, so each exposes
its own native ASGI application factory in its own adapter module. The Import edge is the one
exception: ``ImportService`` requires a ``RoutedSessionProvider`` (implemented only by
...
* NOT AUTHORIZED — ``api_gateway``, ``auth_router`` and ``control_plane`` MUST NOT be imported here,
  because Edge 9 does not require them. Widening the set needs an IC-012 §5.1 amendment, not a code
  edit: a second import-linter ``forbidden`` contract makes the narrow grant machine-enforced;

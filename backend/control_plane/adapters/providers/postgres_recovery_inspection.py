...
* ``database_exists`` / ``inspect_database`` — per-target existence, physical identity
  (``system_identifier`` + ``datname:oid``), and the user-table census (all non-system
  schemas, with row counts) via an admin connection swapped to the target database.
...
# Ordinary tables across all non-system schemas (the census input; read-only catalog scan).
USER_TABLES_QUERY = (
    ...
    def _connect(self, secrets: SecretStore, ref: SecretRef, *, dbname: Optional[str] = None) -> Any:
        ...
        try:
            ...
            if dbname is not None:
                # Swap only the database of the resolved admin descriptor — the target name
                # was validated by the caller's recomputation rule (never caller-supplied).
    ...
    def inspect_database(self, target: str) -> Optional[DatabaseInspection]:
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                names: List[Tuple[str, str]] = [(str(r[0]), str(r[1])) for r in cur.fetchall()]
                user_tables: Dict[str, int] = {}
                for schema, table in names:
                    qualified = f"{schema}.{table}"
                    if _SAFE_CENSUS_IDENTIFIER.fullmatch(schema) and _SAFE_CENSUS_IDENTIFIER.fullmatch(table):
                        ...
                        user_tables[qualified] = -1  # uncountable name -> unknown -> census fails closed

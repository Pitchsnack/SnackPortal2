...
class ProvenanceGraph:
    def __init__(self, provider: LineageReadSessionProvider, *,
                 max_depth: int = 64, max_nodes: int = 1000) -> None:
    ...
    def _walk(self, ctx: RequestContext, lineage_id: str, *, descendants: bool) -> ProvenanceGraphResult:
        ...
        try:
            rows = session.traverse(
                LINEAGE_TABLE, lineage_id, id_col="lineage_id",
                parent_col="parent_lineage_ref", max_depth=self._max_depth,
        ...
        nodes = [
            ProvenanceNode(
                lineage_id=str(r["lineage_id"]), seq=int(r["seq"]),
                event_type=r["event_type"], target_ref=r["target_ref"],
                operation=r["operation"], parent_lineage_ref=r.get("parent_lineage_ref"),
            )
            for r in rows
        ]

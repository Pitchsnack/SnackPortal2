...
class TenantContextResolver:
    ...
    def resolve(
        self, claims: Claims, *, correlation_id: str, carrier_tenant: Optional[str] = None
    ) -> AuthContext:
        active = claims.tenant
        if active is None:
            # No tenant claim -> control-plane-scoped principal (e.g. CONTROL). No active tenant.
            return AuthContext(
                correlation_id=correlation_id, principal_ref=claims.subject,
                active_tenant_id=None, role=None,
            )
        ...
        return AuthContext(
            correlation_id=correlation_id, principal_ref=claims.subject,
            active_tenant_id=active, role=(role.value if role is not None else None),
        )

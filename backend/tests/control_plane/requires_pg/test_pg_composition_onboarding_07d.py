...
# =====================================================================================================
# PRD D-15-T1b — gateway<->database-router dispatch transport pair — SECTION REMOVED.
#
# The proof drove `api_gateway urllib client -> internal database_router HTTP dispatch server ->
# composed DatabaseRouter.route(ctx)`. BOTH ends were deleted with the API Gateway: the Gateway's
# HttpRouterDispatch client and the `POST /internal/dispatch/route` edge existed only to carry a
# Gateway request into the Database Router, and no served Gateway route ever reached that
# fall-through. There is no successor transport to re-aim the proof at, so it is withdrawn rather
# than rewritten — a live proof of a wire that does not exist would be theatre.
#
# What the section proved that STILL MATTERS — that one routed request resolves to exactly one
# physical tenant database — is proved by the onboarding legs above (physically distinct databases
# on the ephemeral cluster) and, at the request layer, by tests/gateway_free/test_adversarial_boundary.py
# (A5/A6/A11) in the default suite.
#
# The run-set count is unchanged: this file remains a single workflow loop entry.
# =====================================================================================================

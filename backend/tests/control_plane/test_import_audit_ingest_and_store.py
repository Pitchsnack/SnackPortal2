"""W1a — Control-Plane Import-audit ingest edge + durable store behavioral contract (DB-free; no driver I/O).

Two halves, both stdlib-only and driver-free (the Driver Containment Standard: this test never imports
``psycopg``; it reaches the store adapter only through its module, with a recording fake connection) — the
one-to-one twin of ``test_gateway_audit_ingest_and_store.py``:

* **Store** (``PostgresImportAuditStore`` behind ``ImportAuditStorePort``): exact SQL parameter mapping to
  ``control_import_audit``, store-assigned ``recorded_at``/``id`` (never caller-bound), idempotent
  ``INSERTED`` / ``DUPLICATE_MATCH`` semantics, same-ID/different-payload conflict rejection (fail closed),
  commit-only-on-new-row transactions, rollback-and-re-raise on failure, the bounded pre-insert validation,
  the closed record field set, and the append-only port surface (exactly one write method; no
  read/update/delete).
* **Ingest** (``build_import_audit_server`` over ``127.0.0.1`` on a test-owned daemon thread — the established
  loopback precedent; production stays thread-free, AT-D15T1-10): strict envelope acceptance/rejection for
  ``POST /internal/import-audit/events``, the exact two-key response envelope for INSERTED / DUPLICATE_MATCH /
  INVALID / CONFLICT / UNAVAILABLE, 404/405 empty-body refusals, forbidden-name and secret-shaped-value
  defenses, the five-action / action-outcome-combination pins, and the no-leak guarantee. The store behind
  the edge is a local test double.

Pure stdlib; standalone-runnable: ``python tests/control_plane/test_import_audit_ingest_and_store.py``.
"""
...
from control_plane.adapters.providers.http_import_audit_api import (  # noqa: E402
    _INGEST_PATH,
    build_import_audit_server,
)
from control_plane.adapters.providers.postgres_store import (  # noqa: E402
    _IMPORT_AUDIT_COLUMNS,
    _IMPORT_AUDIT_SELECT,
    PostgresImportAuditStore,
    _same_instant,
)
from control_plane.import_audit import (  # noqa: E402
...
def _store(conn: _FakeConn) -> PostgresImportAuditStore:
    store = PostgresImportAuditStore(dsn="test-double-descriptor")  # lazy: never connects
    store._conn_cache = conn  # inject the fake into the lazy slot (no I/O anywhere)
    return store


def _record(**overrides: object) -> ImportAuditRecord:
    ...
    return ImportAuditRecord(**base)  # type: ignore[arg-type]


def _stored_row(record: ImportAuditRecord, **overrides: object) -> Tuple[Any, ...]:
    """A stored row in _IMPORT_AUDIT_COLUMNS order, with the timestamptz as a driver datetime."""
    ...
    return tuple(values[name] for name in _IMPORT_AUDIT_COLUMNS)


def test_store01_valid_event_maps_to_exact_sql_parameters() -> None:
    conn = _FakeConn(script=[(1,)])
    record = _record()
    _store(conn).append_import_audit(record)
    sql, params = conn.executed[0]
    assert "INSERT INTO control_import_audit (" + ", ".join(_IMPORT_AUDIT_COLUMNS) + ")" in sql
    ...
    assert params == (
        ...
        "principal:ops",
        "tenant-1",
        "global:GlobalStartupDirectory:g1",
    )


def test_store02_recorded_at_and_id_are_never_caller_bound() -> None:
    conn = _FakeConn(script=[(1,)])
    _store(conn).append_import_audit(_record())
    sql, params = conn.executed[0]
    assert "recorded_at" not in sql, "recorded_at is DB-assigned (DEFAULT now()) — never in the INSERT column list"
    assert len(params) == len(_IMPORT_AUDIT_COLUMNS) == 10
    assert _IMPORT_AUDIT_COLUMNS[0] == "audit_id" and "id" not in _IMPORT_AUDIT_COLUMNS
    assert "recorded_at" not in _IMPORT_AUDIT_COLUMNS
    assert "source_service" in _IMPORT_AUDIT_COLUMNS  # producer constant is store-bound, not a wire field
...
def test_store03_new_event_returns_inserted_and_commits_once() -> None:
    ...
    result = _store(conn).append_import_audit(_record())
    assert result is ImportAuditAppendResult.INSERTED
...
def test_store04_exact_duplicate_returns_duplicate_match_without_commit() -> None:
    ...
    result = _store(conn).append_import_audit(record)
    assert result is ImportAuditAppendResult.DUPLICATE_MATCH
    ...
    assert select_sql == _IMPORT_AUDIT_SELECT and select_params == (record.audit_id,)


def test_store05_same_id_with_changed_field_raises_conflict() -> None:
    record = _record()
    for drifted in (
        _stored_row(record, outcome="error:batch"),
        _stored_row(record, actor_ref="principal:mallory"),
        _stored_row(record, target_ref="tenant-2"),
        _stored_row(record, source_ref="global:GlobalStartupDirectory:g2"),
...
def test_store06_transaction_commits_only_on_valid_insert() -> None:
    conn = _FakeConn()
    try:
        _store(conn).append_import_audit(_record(action="Publish"))
...
def test_store08_validation_enforced_before_any_sql() -> None:
    assert IMPORT_AUDIT_STORE_ACTIONS == (
        "ImportRequested",
        "ImportStarted",
        "ImportResumed",
        "ImportCompleted",
    ...
    for bad in ("Publish", "workspace_memberships_read", "LineageWritten", "import", ""):
        conn = _FakeConn()
        try:
            _store(conn).append_import_audit(_record(action=bad))
    ...
    for bad_version in (0, -1):
        conn = _FakeConn()
        try:
            _store(conn).append_import_audit(_record(event_version=bad_version))
            raise AssertionError("non-positive event_version must be rejected")
        except ImportAuditInvalidError:
            pass
    conn = _FakeConn()
    try:
        _store(conn).append_import_audit(_record(source_service="api_gateway"))
        raise AssertionError("non-import source_service must be rejected (DDL CHECK mirror)")
    except ImportAuditInvalidError:
        pass
...
def test_store09_optional_references_map_to_nullable_columns() -> None:
    conn = _FakeConn(script=[(1,)])
    record = _record(actor_ref=None, target_ref=None, source_ref=None)
    _store(conn).append_import_audit(record)
    _sql, params = conn.executed[0]
    for column in ("actor_ref", "target_ref", "source_ref"):
        assert params[_IMPORT_AUDIT_COLUMNS.index(column)] is None, column


def test_store10_forbidden_model_fields_do_not_exist() -> None:
    names = {f.name for f in dataclasses.fields(ImportAuditRecord)}
    assert names == {
        "audit_id",
        "event_version",
        "occurred_at",
        ...
        "target_ref",
        "source_ref",
    }, "the record field set is CLOSED (10 fields)"
    for forbidden in ("recorded_at", "id", "dsn", "password", "token", "jwt", "request_body", "records", "payload", "hash_chain"):
        assert forbidden not in names, forbidden
    record = _record()
    try:
        record.outcome = "error:batch"  # type: ignore[misc]
        raise AssertionError("ImportAuditRecord must be frozen")
    except dataclasses.FrozenInstanceError:
        pass


def test_store11_no_read_update_delete_method_exists() -> None:
    assert ImportAuditStorePort.__abstractmethods__ == frozenset({"append_import_audit"}), (
        "the store port is append-only write: exactly ONE abstract method"
    )
    public = {n for n in dir(PostgresImportAuditStore) if not n.startswith("_")}
    assert public == {"append_import_audit", "release"}, f"append-only adapter surface violated: {sorted(public)}"
    for verb in ("list", "get", "read", "query", "export", "purge", "update", "delete"):
        assert not any(n.startswith(verb) for n in public), verb
    assert "append_import_audit" not in ControlStore.__abstractmethods__  # frozen ControlStore not widened


def test_store12_construction_requires_exactly_one_source_and_never_connects() -> None:
    for bad in (
        lambda: PostgresImportAuditStore(),
        lambda: PostgresImportAuditStore(dsn="x", ref=object()),
        lambda: PostgresImportAuditStore(ref=object()),
        ...
        try:
            ...
            pass
    store = PostgresImportAuditStore(dsn="test-double-descriptor")
...
class _FakeStore(ImportAuditStorePort):
    ...
    def append_import_audit(self, record: ImportAuditRecord) -> ImportAuditAppendResult:
        ...
        if self.behavior == "conflict":
            raise ImportAuditConflictError("replayed audit_id with a different payload")
        if self.behavior == "invalid":
            raise ImportAuditInvalidError("record rejected")
        raise RuntimeError("internal-store-detail-that-must-never-leak")


@contextlib.contextmanager
def _serving(store: ImportAuditStorePort) -> Iterator[str]:
    server, base_url = build_import_audit_server(store)  # 127.0.0.1, ephemeral port
...
# A single-threaded stdlib HTTP/1.0 server closes the connection immediately after a bodiless 404/405 refusal,
# which on Windows loopback intermittently surfaces client-side as ConnectionAbortedError/ConnectionResetError.
# Bounded retry, SOLELY for the side-effect-free refusal probes (they never reach the store); every other
...
def test_ingest01_valid_insert_envelope_accepted() -> None:
    ...
    assert record.audit_id == _AUDIT_ID and record.action == "ImportCompleted"
    assert record.source_service == "import_service"  # store-side producer constant, not from the wire
    assert record.actor_ref == "principal:ops" and record.target_ref == "tenant-1"
    assert record.source_ref == "global:GlobalStartupDirectory:g1"
...
def test_ingest10_wrong_field_types_rejected() -> None:
    with _serving(_FakeStore("insert")) as base:
        ...
        _expect(base, _envelope(_event(target_ref=None)), 400, "INVALID")
        _expect(base, _envelope(_event(source_ref="")), 400, "INVALID")
        _expect(base, json.dumps({"version": 1, "event": [1]}).encode(), 400, "INVALID")


def test_ingest11_action_outcome_combination_pins_enforced() -> None:
    # W1a persists all five import actions, each with its pinned outcome vocabulary.
    with _serving(_FakeStore("insert")) as base:
        # lawful combinations accepted (INSERTED):
        for action in ("ImportRequested", "ImportStarted", "ImportResumed"):
            _expect(base, _envelope(_event(action=action, outcome="success")), 200, "INSERTED")
        _expect(base, _envelope(_event(action="ImportCompleted", outcome="success")), 200, "INSERTED")
        _expect(base, _envelope(_event(action="ImportCompleted", outcome="replayed")), 200, "INSERTED")
        _expect(base, _envelope(_event(action="ImportFailed", outcome="error:batch")), 200, "INSERTED")
        _expect(base, _envelope(_event(action="ImportFailed", outcome="error:unsupported_source")), 200, "INSERTED")
        # unlawful combinations rejected at the edge:
        _expect(base, _envelope(_event(action="ImportRequested", outcome="replayed")), 400, "INVALID")
        _expect(base, _envelope(_event(action="ImportStarted", outcome="error:x")), 400, "INVALID")
        _expect(base, _envelope(_event(action="ImportCompleted", outcome="error:x")), 400, "INVALID")
        _expect(base, _envelope(_event(action="ImportFailed", outcome="success")), 400, "INVALID")
        _expect(base, _envelope(_event(action="ImportFailed", outcome="error:")), 400, "INVALID")  # empty suffix
        _expect(base, _envelope(_event(action="ImportFailed", outcome="error:has space")), 400, "INVALID")
        _expect(base, _envelope(_event(action="Hacked", outcome="success")), 400, "INVALID")  # unknown action
        _expect(base, _envelope(_event(action="LineageWritten", outcome="success")), 400, "INVALID")  # excluded


def test_ingest12_forbidden_names_and_secret_shapes_rejected() -> None:
    with _serving(_FakeStore("insert")) as base:
        for forbidden in ("id", "recorded_at", "source_service", "dsn", "token", "password", "records", "connection"):
...
if __name__ == "__main__":
    _h.run(
        [
            ...
            test_ingest11_action_outcome_combination_pins_enforced,

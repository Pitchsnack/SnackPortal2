...
| **CONF-9** | `host="0.0.0.0"`, `reload=True`, missing uvicorn flags | **RESOLVED, then FINALIZED by [D-47](D-47-AI-Ownership-Cardinality-And-Service-Exposure-Model.md) §2** on the **BIND vs PUBLISH** distinction (IC-013 §21.1 E-1…E-7): **only the BFF may be publicly reachable/published**; internal services default to loopback/private in local development; **internal services may bind inside private runtime/container namespaces as required, but must not be publicly published**; `reload=True` local-development only; serving configuration (process, logging, proxy-header, server-header) is **environment-specific with secure defaults** — deployment settings, not architecture invariants (E-5, amended 2026-08-21). *(D-46's original bind-only wording — "`0.0.0.0` prohibited outside a deliberately-configured deployment binding" — targeted the wrong layer and is superseded by D-47 §2.)* |
...
- **CONF-4 is CLOSED** — ratified by Dan on 2026-08-21 as Option A and recorded at **D-47 §1**; §8 below is retained as the historical options analysis. IC-008 is **amended by D-47**.
...
| **IC-008** — Ownership | **No change by D-46.** Subsequently **amended by [D-47](D-47-AI-Ownership-Cardinality-And-Service-Exposure-Model.md) (2026-08-21)** — AI-ownership cardinality ratified (CONF-4 closed): at most one *current* AI Owner, single reference; contribution ≠ ownership; Option C reserved. Remains `Final`. |
| **IC-001, IC-002, IC-003, IC-004, IC-011** | **No change** — boundaries respected |

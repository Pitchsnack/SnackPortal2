...
| **E-5** | **Served-invocation hygiene flags are a set:** `--workers 1 --no-access-log --no-server-header --no-proxy-headers`. Omitting one silently restores a uvicorn default; a partial application is a violation. (`--factory` is a start-path choice, not a hygiene control, and is **not** mandated — see IC-013 §21.) |

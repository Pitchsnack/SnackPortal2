...
| **E-5** | **Serving configuration is environment-specific, with secure defaults.** *(Amended 2026-08-21 — see the E-5 note below.)* Uvicorn process, logging, proxy-header and server-header settings are **deployment settings, not architecture invariants**: selected per environment from secure defaults (one process per service; access logging off; proxy headers off/untrusted; server header off, recommended). Binding: a setting left unconfigured takes the secure default; proxy headers require an **explicit trusted-proxy boundary**; a forwarded header is **never** a carrier, routing authority, or identity source (§5/§7 prevail); access logs inherit the references-only discipline; production serving configuration MUST be documented and testable. |

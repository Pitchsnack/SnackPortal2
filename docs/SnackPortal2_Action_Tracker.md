...
| 23c | **Deployment-manifest exposure check** — prove exactly one service publishes a port and it is the BFF, across every compose file, K8s manifest, env template and launcher. E-1/E-3 are violated by *configuration*, so a code-only test set cannot discharge them | **High** | ⬜ | Phase 1 acceptance (D-47 §2, IC-013 §21.1 E-6) |

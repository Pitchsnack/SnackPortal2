...
- The Database Router is the **sole authority on tenant-database resolution**: it alone determines which single physical database a request may reach, registry-authoritatively from the signed claim. **A tenant-resident domain service MAY hold the connection it opens**, but only under a Database Router connection grant, and only if that service is on the router's explicit grant allowlist. **The BFF is never on that allowlist**, and neither is the Access Control Service (IC-014 §6 is unchanged and absolute). *(Amended 2026-08-23 by [D-48](../docs/D-48-Tenant-Connection-Ownership.md) §4, closing rebuild finding N-4: resolution AUTHORITY and connection CUSTODY are separated; constraints C-1…C-4 of D-48 are normative. The previous wording — "the Database Router remains the **only** service permitted to open a tenant database" — conflated the two, and could not be literal once the architecture became fourteen separately-deployed processes. **No isolation rule is weakened**: one request still resolves to exactly one physical database, there is still no Control-DB fallback, and §4 / §11 / §21.1 are unchanged.)*

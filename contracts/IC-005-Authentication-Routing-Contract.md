...
**`CarrierMismatch` durable homing — cross-reference (added 2026-07-27, Post-10C.3 corrective, D-43).** References-only cross-reference: the **signed tenant claim remains the sole routing authority**, and the carrier remains **match-or-reject only** — never authorization. A carrier/claim mismatch is denied fail-closed **before tenant routing** (no Database Router and no tenant-database contact), and the gateway-edge `CarrierMismatch` denial record is **durably homed in Control-DB operational audit** (IC-002 class 3 residency; the durable shape, the sole-emitter rule, and the fail-closed 403/503 semantics are defined in IC-010, D-43) carrying the REQUIRED **opaque, length-bounded `carrier_ref`** (never parsed, resolved, or trusted). **No tenant routing occurs on a mismatch.** Token validation, membership law, and active-tenant law are unchanged.
...
## CLM Controlled-Local Rehearsal Authority (2-Day Accelerated Controlled Local MVP, D-42)

*Added under D-42 (the 2-Day Accelerated Controlled Local MVP V2 START-GATE, Stage A). Additive only — no normative change to the Two-Phase Authentication Model, D-05 OIDC stateless JWT validation, JWKS, the D-32 role hierarchy, D-06 tenant carriage, the D-33 carrier match-or-reject rule, the R1 Principal-Only Bootstrap & OIDC Transition section above, the Runtime Operational Audit Emission section, or any denial semantics. CLM names the bounded controlled-local rehearsal slice this section governs; every R1 statement above remains true within the R1 slice. For the CLM rehearsal only, this section supplies the separate governance the R1 section's closing bullet anticipates for tenant-scoped access.*

- **CLM slice definition.** CLM is the D-42 controlled-local rehearsal slice: one synthetic-data journey — controlled-local OIDC login → principal-only Gateway authentication → `GET /memberships` → backend-validated tenant selection → one tenant Startup read → one bounded tenant Startup update → one unauthorized-tenant denial → audit evidence → rollback and restore — executed against local fixtures only. It is not a production or staging activation and it uses no real customer data; every principal, membership, tenant, and Startup row in the rehearsal is synthetic and local.
- **Keycloak ratified for the controlled-local CLM rehearsal.** Keycloak remains the selected OIDC provider for the controlled-local CLM rehearsal, extending the R1 controlled-proof selection above to the full CLM journey (login, memberships, the two CLM tenant Startup routes recorded in IC-010, the unauthorized-tenant denial, and the rollback evidence). Validation remains asymmetric-only (`allowed_algs ⊆ {RS256, RS384, RS512, ES256}`; `HS*` and `none` rejected at configuration), trusted via the static `SP2_AR_ISSUERS` entry. This ratification is controlled-local only: it effects no permanent production-provider decision, activates no production IdP, **does not make Keycloak the permanent production IdP**, and the contract remains provider-neutral (any OIDC-standard, self-hostable IdP satisfies it).
- **Tenant-scoped access for the rehearsal — minted only at the IdP.** The CLM tenant Startup routes require an active tenant context. For the rehearsal, a tenant-claim-bearing access token is **minted only at the IdP** (a standard Authorization Code + PKCE login or re-authentication whose issued token carries the signed `tenant` claim naming the selected tenant); the signed claim binds that tenant under the existing D-06 match-or-reject rule, unchanged. **The Gateway mints and exchanges no token.** The B2 workspace-switch protocol, RFC 8693, and any Gateway-issued tenant-scoped token remain out of CLM scope and separately governed.
- **Backend-validated tenant selection.** The browser may request a tenant; lawfulness is decided exclusively by the backend. Before any routing, the platform verifies the authenticated principal is an authorized member of the claimed tenant (D-04, existing law), and the Database Router alone resolves the physical database from the signed claim (D-07). No browser value — carrier, body field, cookie, query string, portal or workspace state — selects a tenant or a database.

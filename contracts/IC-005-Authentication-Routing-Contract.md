...
- **CLM audit evidence.** The CLM rehearsal's audit evidence set is the four events recorded in IC-010's CLM section: `workspace_memberships_read` (unchanged definition above), `tenant_startup_read`, `tenant_startup_update`, and the gateway-edge `RouteDenied` denial record. Runtime emission of these events is implemented only under the Stage B implementation slice of the D-42 START-GATE and is not yet emitted by the current runtime.

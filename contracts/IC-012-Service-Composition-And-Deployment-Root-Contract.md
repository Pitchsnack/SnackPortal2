...
**Amendment (2026-08-21, D-46 — Option A Gateway-Free Target Architecture Ratification; resolves Phase 0 finding CONF-11):** **scope re-point; the principles are carried forward, the Edge-9 specifics become historical.** The Option A target replaces the nine-edge topology this contract was written against with the `src/snackportal2/services/*` layout, so §5's `deployment.import_edge:create_app_from_env` authority, §16's exact package placement, and the enumerated Edge-9 service list (`import_service`, `database_router`, `lineage_service`, `shared`) are marked **HISTORICAL** — they describe the superseded topology and bind no Option A implementation. **Everything that made this contract worth having is carried forward unchanged and re-scoped to the Option A `deployment/` root**, namely: *(a)* cross-service composition sits **ABOVE** the services — the root may import them; **nothing may import it** (§3/§4); *(b)* its cross-service authority is **narrow and exhaustive**, never general — widening it requires an amendment (§5.1), not an unreviewed import; *(c)* it is **composition-only** — no domain logic, no business rules, no SQL, no authorization policy, no tenant-selection policy, no lineage semantics, no routing authority (§17); *(d)* it **selects no database** — it holds no DSN, no tenant→database mapping, no naming convention, no override; registry-authoritative resolution stays inside the Database Router (§6/§9); *(e)* a live routed tenant session is a **transactional handle that cannot cross HTTP** and MUST remain in-process (§8/§10); *(f)* **fail-closed startup** (§11); *(g)* services remain **mutually independent** and the root is **analyzed, not merely unlisted**, so it can never become a silent back-channel between two services (§13/§18). **The root is not an ingress** — it exposes no surface, serves no request, and is reachable by no caller; under the Option A architecture the **FastAPI BFF** is the single frontend-facing ingress (IC-013), and §15's reference to the API Gateway as "sole served ingress" resolves to the BFF. **The import-linter enforcement requirement (§13) survives in full** and MUST be re-expressed against the Option A package graph before Phase 1 acceptance. Status remains **Draft / Proposed**; revision remains IC-012-DRAFT-1. **No runtime is implemented by this amendment**; production remains NOT READY / DO-NOT-ACTIVATE.

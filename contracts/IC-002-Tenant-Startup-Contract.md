...
**Amendment (2026-07-17, PRD B5-BLK-6C-A):** implementing Dan's **Option A** audit-obligation reconciliation (`DECIDE B5-BLK-6C-A OPTION A`). The existing *MembershipsForPrincipal* audit MUST (D-33 workspace-related audit events) is **preserved and homed**: the *Audit-Section Extension* gains the public-edge **success-access subclass (3b)** — action `workspace_memberships_read`, **route-owning public edge emitter**, **Control-DB operational audit** home, references-only minimum shape — mandating **exactly one** event per successful self-scoped enumeration, **including a successful empty enumeration**. The four public-edge denial/anomaly classes and the router-edge routing-decision subclass (3a) are unchanged. *(Amended 2026-08-11 under **D-45**: the emitter is the route-owning authenticated public edge and the subclass label “gateway-edge” becomes “public-edge” — a **label and emitter-identity change only**; no action string, class membership, exactly-once rule, minimum shape, or Control-DB residency is altered.)* **Additive; the four Core Invariants are untouched verbatim.** Contract-only (contracts precede code): runtime emission remains pending **B5-BLK-6C-B**; B5-BLK-6C-A adds no runtime implementation and closes no blocker.
...
- **D-33 workspace-related audit events** *(added 2026-06-12, PRD-CAP-01A)* — the following control-plane audit obligations, aligned with the **Control-DB operational-audit model approved by D-34** and bound by its **Global Audit Representation Rule** (references only — never names, emails, PII, or payloads):
  ...
  - **MembershipsForPrincipal** calls MUST be audited (actor, subject principal reference, timestamp, correlation id — references only). *Homed 2026-07-17 under B5-BLK-6C-A (Dan-authorized Option A): this obligation is bound by the public-edge success-access subclass **(3b)** in the *Audit-Section Extension* below — action `workspace_memberships_read`; route-owning public edge emitter (D-45); Control-DB operational audit.*
...
**(3) Runtime Operational Audit — Control-DB resident.** `RouteDenied`, `CarrierMismatch`, `CarrierOnControlAnomaly`, `IsolationAnomaly` — the runtime anomaly/denial classes emitted at the request edge (emission owned by IC-005/IC-010 — see IC-005 *Runtime Operational Audit Emission*; this section is their **residency/retention home**). Fields: `audit_id`, `action` ∈ {RouteDenied, CarrierMismatch, CarrierOnControlAnomaly, IsolationAnomaly}, `correlation_id`, `outcome`, `actor_ref` (optional), `tenant_ref` (optional), `carrier_ref` (optional; **opaque, length-bounded** carrier identifier per IC-005:116 — never parsed/resolved), `timestamp`. Control-DB resident; emitted **once per correlation id** (single-edge — IC-010 §J). *(Field set matches the as-built `EdgeAuditEvent`, `backend/shared/public_edge.py`. Amended 2026-08-11 under **D-45**: this is a **source-pointer refresh only** — the as-built type moved from the deleted `api_gateway` package to the shared public-boundary kernel with **no field, action string, or shape change**. The durable store's `source_service = 'api_gateway'` producer constant and the `control_gateway_audit` table name are **frozen compatibility artifacts** pending a separately authorized DDL/SecretRef migration, and are not evidence that an API Gateway runtime exists.)*

**(3a) Runtime Operational Audit — Database Router edge (router-edge routing-decision subclass; added 2026-07-13, DBR-AR-2A).** Additive subclass **within class 3** (Runtime Operational Audit). The public-edge event set in (3) above is **unchanged and remains owned by the route-owning public edge**; this subclass homes the **router-edge routing-decision events**, whose emission edge is reconciled in [IC-005](IC-005-Authentication-Routing-Contract.md) *Runtime Operational Audit Emission*:
...
- **Event identity is (edge/subclass, action).** `RouteDenied` and `IsolationAnomaly` appear in both the public-edge set (3) and this router-edge subclass as **different events on different edges**. **No event is emitted by more than one component.** Public-edge events and Database-Router-edge events are different subclasses with different sole emitters. The Authentication Router is **not an emitter of record** for any class-3 event (detection and signalling only — IC-005).
...
**(3b) Runtime Operational Audit — Public-edge success-access subclass (`workspace_memberships_read`; added 2026-07-17, B5-BLK-6C-A, Dan-authorized Option A; emitter retargeted 2026-08-11, D-45).** Additive subclass **within class 3** (Runtime Operational Audit; Control-DB resident), homing the audit MUST that *Audit Requirements* (D-33 workspace-related audit events) states for **MembershipsForPrincipal**. It is a **separate success-access subclass**: the four public-edge denial/anomaly classes remain exactly the historic four — `RouteDenied`, `CarrierMismatch`, `CarrierOnControlAnomaly`, `IsolationAnomaly` — unchanged, and the router-edge routing-decision subclass (3a) is unchanged.
...
- **Emitter.** **The route-owning public Workspace edge is the emitter**; it is the **sole emitter of this success event** (single edge). The Auth Router remains detection and signalling only. The Database Router's router-edge subclass (3a) is unaffected.
- **Home.** The event resides in **Control-DB operational audit** as a public-edge success-access subclass. It is a success-access event and MUST NOT be relabelled or classified as a denial, anomaly, or routing event.
...
**`CarrierMismatch` durable homing — cross-reference (added 2026-07-27, Post-10C.3 corrective, D-43).** References-only cross-reference: the tenant Startup route's carrier-mismatch denial (403 `carrier_mismatch`) is **durably homed in Control-DB operational audit** (class 3 residency above, unchanged) **before tenant routing** — the denial is pre-context and pre-routing, no Database Router and no tenant-database contact occurs, and the durable record carries the REQUIRED **opaque, length-bounded `carrier_ref`** (IC-005:116 — never parsed, resolved, or trusted) with **no tenant or record reference**. The durable shape, the sole-emitter rule, and the fail-closed 403/503 semantics are defined in [IC-010](IC-010-API-Gateway-Contract.md) (D-43). This cross-reference alters no Startup read/update authority, no DTO, and no route behavior.
...
**DEC-11 resolution.** With the tenant-resident ownership-audit class now contractually homed (class 2 above), the blocker recorded as **DEC-11** — *"the API Gateway PRD must not bind ownership-audit DTO residency until the IC-002 ownership-audit-section extension lands"* (ADR IC-009-ADOPT; quoted verbatim as the historical blocker text — under D-45 read "the API Gateway PRD" as "the public-edge execution PRD") — is **resolved by the landing of this section.** Binding the IC-009 `AuditEventDTO` residency to this home is a downstream portal / public-edge execution item; this section makes that binding *permissible*, it does not perform it.

**Execution-deferred (contracts precede code).** Audit **persistence** (the Control-DB and tenant-DB audit stores), the public edge's audit **sink** wiring, and any emission **tests** beyond those already passing are **execution-PRD items** — none authorized here.
...
*`MembershipsForPrincipal` (added 2026-06-12 per D-33-E1 Item 2) is **contract-owned here**; its implementation is deferred to the public-edge / frontend-integration execution PRD — contracts precede code.*

...
**Amendment (2026-08-21, D-46 — Option A Gateway-Free Target Architecture Ratification; resolves Phase 0 finding CONF-2):** **composing-seam re-point only.** IC-009's DTOs were contractually *gateway-composed* under IC-010 §V, and IC-010 is now **Superseded** by **IC-013 — BFF Ingress Contract** + **IC-014 — Access Control Contract** (D-46 §2). Wherever this contract names the **API Gateway** as the composing seam, the composing seam is now the **FastAPI BFF**, and wherever it cites **IC-010 §V** for response composition, the governing section is now **IC-013 §19** (whose rules are carried forward unchanged: arbitrary downstream body pass-through remains **FORBIDDEN**; contract-approved DTO composition remains permitted only under the §V.1 conditions, restated at IC-013 §19). Correspondingly, §F's "Gateway-Facing Portal API Contracts" map onto **IC-013 §16** (the enumerated BFF operation taxonomy) rather than IC-010 §Q, and §I's workspace-switcher reference resolves to IC-013 §6/§7. **Nothing else changes.** The **DTO catalogue, every DTO's field set, the D-37 §10 provenance markers, the D-35 anonymity precedence, the §C per-role × per-directory visibility matrix, the §D `SharedItemReferenceDTO` (still authored-but-inert until IC-007 is Final), the §M deferrals, and the §P/§R verification criteria are all unchanged**, and this contract's **revision identity remains `IC-009-R1`** — downstream PRDs citing IC-009-R1 remain correct. Status remains **Final**. No new DTO, no new portal class, no new role, no new visibility cell, and no positive sharing or AI capability is introduced. **No runtime is implemented by this amendment**; production remains NOT READY / DO-NOT-ACTIVATE.
...
- **Gateway-only for the CLM journey.** Every CLM read and write traverses the API Gateway; the CLM journey performs **no direct Supabase business-data access** and receives no database credential, SecretRef, or physical routing detail (Boundary, unchanged).
- **Code-closure note.** The as-built portal DTO union and catalogue stay closed at their current pins until the Stage B implementation slice, which MUST extend the binding guards (including the tenant-resident `lineage_reference` binding) in the same change that widens them.

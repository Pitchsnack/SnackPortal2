...
**Amendment (2026-08-21, D-47 — AI Ownership Cardinality Ratification; closes Phase 0 finding CONF-4):** **Dan ratified Option A.** Phase 0 recorded that three sources disagreed on AI-ownership cardinality and representation — this contract (*at most one*, nullable `owner_ai_agent_ref` **on the record**), the tenant DDL `006_ownership.sql` (*zero or more distinct*, via composite-PK `{entity}_ai_ownership` **join tables**), and Canonical Overview locked invariant #3 ("one AI owner"). **This contract prevails.** The ratified rule is: **the SnackPortal2 MVP supports AT MOST ONE *CURRENT* AI Owner per record**, carried as the single nullable `owner_ai_agent_ref` reference defined in the *AI Ownership Rule* below. **Multiple AI Agents MAY contribute to a record; contribution is NOT ownership.** Contribution is recorded through **task history, provenance (IC-004 lineage) and operational audit (IC-002)** and **MUST NOT** create a second AI ownership, a second owner reference, a co-owner, or any ownership-shaped record. **"Current" is normative and is a cardinality bound, not a permanence claim:** AI ownership MAY pass from one AI Agent to another over time — at most one holds it at any instant — and each such succession is an audited ownership transfer under the *Ownership Transfer* rules, with history retained. It is **never** a widening to two concurrent AI owners. **Option C (a separate, explicitly non-ownership AI-involvement relation, additive) is RESERVED** for a later decision should Option A Phase 9 prove it necessary; reserving it authorizes nothing and creates no field, table, or capability now. **What this amendment does NOT change:** `owner_ai_agent_ref` remains **nullable, optional, reserved, and NULL platform-wide until IC-006** defines the AI-agent identity namespace and its resolution path (V10 stands — this amendment settles *cardinality*, not *activation*); the human ownership rule (exactly one `owner_agent_ref`) is untouched; and all five Ownership Principles (≠ Authorization, ≠ Routing, ≠ Residency, ≠ Authentication, ≠ Visibility) are unweakened. **Known divergence, recorded not resolved:** the tenant DDL still implements the zero-or-more join-table shape, and `tests/architecture/test_tenant_ddl_schema_guards.py::test_ownership_pk_shapes` **actively asserts** the composite `PRIMARY KEY ({entity}_id, ai_agent_id)` — so a guard currently enshrines the shape this amendment rules against. Reconciling the DDL and that guard is **named Phase-7 work** (D-47 §4); no DDL or test was changed by this amendment. Status remains **Final**. **No runtime is implemented**; production remains NOT READY / DO-NOT-ACTIVATE.
...
**Cardinality ratified (D-47, 2026-08-21 — closes CONF-4).** *At most one **current** AI Owner per record*, held in the single `owner_ai_agent_ref` reference above.
- **Additionally prohibited by this ratification:** two or more concurrent AI owners on one record; a co-owner or secondary-owner reference; an ownership-shaped join table for AI ownership; and any representation that permits a set of AI owners rather than a single reference.
- **Contribution ≠ Ownership.** Any number of AI Agents MAY contribute to a record. A contribution record MUST be a **task-history, provenance (IC-004) or operational-audit (IC-002)** record. It MUST NOT be an ownership record, MUST NOT populate `owner_ai_agent_ref`, MUST NOT be counted as ownership by any consumer, and MUST NOT confer any authority — contribution inherits Ownership Principle 1 (**≠ Authorization**) *a fortiori*: if owning a record grants nothing, contributing to one certainly grants nothing (IC-014 §7).
- **Succession is permitted and audited.** AI ownership MAY move from one AI Agent to another; at most one holds it at any instant. Each move is an audited ownership transfer (*Ownership Transfer*), and history is retained in the audit/provenance record — never by widening the live reference.
- **Contract gap — the contribution record has no home yet.** "Task history" names a record class that **does not exist** in any contract or schema today. Provenance (IC-004) and operational audit (IC-002) both exist; a *task-history* record class does not. Specifying it is **named prerequisite work** attaching to IC-006 / the AI Agent Service (D-47 §4). Until it is specified, contribution is recordable **only** through the existing IC-004 provenance and IC-002 operational-audit classes, and **no new record class may be invented at implementation time to fill the gap**.
- **Option C reserved.** A separate, explicitly non-ownership AI-*involvement* relation remains available as a purely **additive** later decision if Phase 9 proves it necessary. Reserving it authorizes nothing, creates no field or table, and does not weaken the at-most-one-current rule in the meantime.
